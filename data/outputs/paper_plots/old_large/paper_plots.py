...
def check_diversity_tsne(scaled_real, scaled_synthetic, model_name, plot_real_on_top=False):
    ...
    plt.xlabel('$1^{{st}}$ t-SNE component', fontsize=16)
    plt.ylabel('$2^{{nd}}$ t-SNE component', fontsize=16)
    plt.tick_params(axis='both', labelsize=12)
...
#     ax.set_title('Class balance: real vs. synthetic data', fontsize=14)
    ...
    legend2 = ax.legend(handles=[hatch_patch], loc='upper center', bbox_to_anchor=(0.5, 0.97), 
...
#     ax.invert_yaxis()
    ax.set_ylim(-0.6, len(df_plot['Model']) + 0.65)  # add space above and below
    plt.tight_layout()
...
def utility_augmentation_size():
    ...
    df_concat['model_sort_order'] = df_concat['Model'].map(order_mapping)
    # Sort by model order first, then by "Size before cleaning"
    df_concat = df_concat.sort_values(by=["model_sort_order", "Size before cleaning"]).reset_index(drop=True)
    # Drop the temporary sort column
    df_concat = df_concat.drop('model_sort_order', axis=1)
    ...
    # print(f"Final dataframe shape: {df_concat.shape}")

    # ============================(For paper)===========================
    # List of models to remove
    remove_models = ["Default TVAE", "Default CTGAN", "Default CopulaGAN"]
    # Filter the DataFrame
    df_concat = df_concat[~df_concat["Model"].isin(remove_models)]
    df_concat["Model"] = df_concat["Model"].apply(rename_model)
    ...
    for i in ["before", "after"]:
        for metric in ["PR-AUC", "Normalised MCC"]:
            ...
            trtr_val  = df_concat[df_concat["Model"]=="TRTR"][metric].iloc[0]
            ...
            palette = sns.color_palette("tab10", n_colors=df_concat["Model"].nunique())
            # Map models (exclude TRTR) -> colors
            model_colors = dict(zip(df_concat.iloc[1:]["Model"].unique(), palette))

            # Plot PR-AUC & Normalised MCC vs Size before & after cleaning (light markers)
            sns.lineplot(
                data=df_concat.iloc[1:], # plot from second row and skip row with TRTR values
                x=f"Size {i} cleaning", 
                y=metric, 
                hue="Model", 
                marker="o", 
            ...
            for model in df_concat.iloc[1:]["Model"].unique():   # remove TRTR from the list of models
                sub = df_concat[df_concat["Model"] == model]
            ...
            y_max = max(df_concat[metric].max(), trtr_val)
            y_min = min(df_concat[metric].min(), trtr_val)
